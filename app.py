...
.page-wrap {
  ...
  flex-direction: column;
  align-items: center;
  padding: 80px 0 40px; /* 위쪽 UI 공간 확보 */
  box-sizing: border-box;
}
...
.skel-line.w1 { width: 60%; }
.skel-line.w2 { width: 40%; }
.skel-line.w3 { width: 80%; }
.shimmer::after {
  content: "";
  position: absolute;
  top: 0;
...
#cards {
  ...
  padding: 0 24px 8px;  /* 좌우 여백 + 아래 여백 */
  gap: 35px;            /* 카드 사이 공백 */
}
...
.reaction-overlay {
  position: absolute;
  inset: 0;                          /* 카드 전체 덮기 */
  background: rgba(0, 0, 0, 0.25);   /* 반투명 회색 */
  display: flex;
  align-items: center;
  justify-content: center;
  opacity: 0;
  ...
  z-index: 2;                        /* 카드 내용 위로 올리기 */
}

/* show 클래스를 붙였을 때만 보이게 */
.reaction-overlay.show {
  opacity: 1;
}

/* show 클래스가 붙었을 때만 보이게 */
.reaction-overlay.show {
  opacity: 1;
}
...
<div class="page-wrap">
  <div id="loading">현재 위치를 불러오는 중입니다...</div>
  ...
  <div id="skeleton">
    <div class="skel-card">
      ...
      <div class="skel-line w1 shimmer"></div>
      <div class="skel-line w2 shimmer"></div>
      <div class="skel-line w3 shimmer"></div>
    </div>
  </div>

  <!-- 캐러셀 영역: [←] [카드들] [→] -->
  <div class="carousel-wrap">
    <button class="nav-arrow" id="arrowLeft">‹</button>
    <div id="cards"></div>
    <button class="nav-arrow" id="arrowRight">›</button>
  </div>
...
function start() {
  ...
  // 1. geolocation 자체가 없는 경우 → 바로 기본 좌표로
  if (!navigator.geolocation) {
    loadingEl.innerText =
      "브라우저에서 위치 정보를 지원하지 않습니다. 기본 위치로 추천합니다.";
    fetchReco(33.462046, 126.329235); // 제주 기본 좌표
    return;
  }
  ...
  function go(lat, lon) {
    if (called) return;
    called = true;
    console.log("[RECO] 위치 사용:", lat, lon);
    fetchReco(lat, lon);
  }

  // 2. 일부 브라우저에서 SecurityError 등 예외가 바로 나는 경우를 막기 위해 try/catch
  try {
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        // 정상적으로 위치 받아온 경우
      ...
      (err) => {
        // 콜백으로 에러가 들어오는 일반적인 경우
        console.error("[RECO] 위치 오류(콜백):", err);
        loadingEl.innerText =
          "위치를 가져오지 못했습니다. 기본 위치로 추천합니다.";
        go(33.462046, 126.329235); // 기본 좌표
      },
      {
    ...
    // 여기로 들어오면 getCurrentPosition 자체가 예외를 던진 것
    console.error("[RECO] 위치 API 예외(try/catch):", e);
    loadingEl.innerText =
      "위치 정보를 사용할 수 없습니다. 기본 위치로 추천합니다.";
    go(33.462046, 126.329235);
  }

  // 3. geolocation이 너무 오래 멈춰 있는 경우 (콜백도, 예외도 안 온 경우) 최종 타임아웃
  setTimeout(() => {
    if (!called) {
      console.warn("[RECO] 위치 응답 지연 → 기본 좌표 사용");
      loadingEl.innerText =
        "위치 응답이 느립니다. 기본 위치로 추천합니다.";
      go(33.462046, 126.329235);
    }
  }, 8000);
}
...
function fetchReco(lat, lon) {
  // 로딩 시작: 텍스트 + 스켈레톤 ON, 카드/제목 OFF
  loadingEl.style.display = "block";
  loadingEl.innerText = "주변 맛집을 불러오는 중입니다...";
  ...
  fetch("/api/reco", {
    ...
    .then(showCards)
    .catch((err) => {
      console.error("[RECO] API 오류:", err);
      skeletonEl.style.display = "none";
      loadingEl.style.display = "block";
      loadingEl.innerText =
        "추천 정보를 불러오지 못했습니다. 새로고침 후 다시 시도해주세요.";
      if (titleEl) titleEl.style.display = "none";
    });
}
...
function showCards(data) {
  ...
  if (!data || !data.length) {
    loadingEl.style.display = "block";
    loadingEl.innerText = "근처 추천 맛집을 찾지 못했습니다.";
    if (titleEl) titleEl.style.display = "none";
    return;
  }
  ...
  data.forEach((item) => {
    ...
    const distanceText =
      item.distance_km != null ? `약 ${item.distance_km}km` : "";
    ...
    const kakaoLines = [];
    if (item.address) kakaoLines.push("📍 " + item.address);
    if (item.open_info) kakaoLines.push("⏰ " + item.open_info);
    const kakaoInfo =
      kakaoLines.join("<br>") ||
      "카카오맵에서 상세 정보를 확인할 수 있어요.";
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="badge-row">
          <span class="badge">${item.category || "기타"}</span>
        ...
        <div class="btn-row">
          <button class="btn btn-like">👍 좋아요</button>
          <button class="btn btn-dislike">👎 별로예요</button>
          <button class="btn btn-map">카카오맵으로 이동</button>
        </div>
      </div>
    `;
    ...
    mapBtn.addEventListener("click", () => {
      if (!item.place_id) {
        console.warn("place_id가 없습니다:", item);
        alert("카카오맵 정보가 없는 가게입니다.");
        return;
      }
      const kakaoUrl = `https://place.map.kakao.com/${item.place_id}`;
      window.open(kakaoUrl, "_blank");
    });
  });

  // 카드 DOM 생성 끝난 뒤 한 번만 세팅
  cardEls = Array.from(cardsEl.querySelectorAll(".card"));
  setupPager(cardEls.length);
  setupCarousel();
  scrollToIndex(0); // 첫 카드 중앙 정렬
}
...
function updatePager(idx) {
  const dots = pagerEl.querySelectorAll(".pager-dot");
  dots.forEach((d, i) => {
...
function scrollToIndex(idx) {
  ...
  if (idx < 0) idx = 0;
  if (idx >= cardEls.length) idx = cardEls.length - 1;

  const containerRect = cardsEl.getBoundingClientRect();
  const cardRect = cardEls[idx].getBoundingClientRect();

  const containerCenter = containerRect.width / 2;
  const cardCenter = cardRect.width / 2;

  const currentLeft = cardsEl.scrollLeft;
  const offsetLeft = cardRect.left - containerRect.left;
  const targetLeft =
    currentLeft + offsetLeft - (containerCenter - cardCenter);
...
function updateArrows() {
  ...
  arrowRight.disabled = currentIndex >= cardEls.length - 1;
}

function setupCarousel() {
  updateArrows();

  arrowLeft.onclick = () => scrollToIndex(currentIndex - 1);
  arrowRight.onclick = () => scrollToIndex(currentIndex + 1);
  ...
  if (cardEl) {
    ...
    if (overlay && emoji) {
      // 좋아요 / 싫어요에 따라 이모지 변경
      emoji.textContent = isLike ? "😆" : "😢";

      // 오버레이 보이게
      overlay.classList.add("show");

      // 애니메이션 다시 시작 (pop 클래스 리셋)
      emoji.classList.remove("pop");
      void emoji.offsetWidth;    // reflow 강제
    ...
    // 2) 다음 카드로 자동 이동
    // cardEls 배열에서 현재 카드 인덱스 찾기
    const idx = cardEls.indexOf(cardEl);
    if (idx !== -1 && cardEls.length > 1) {
      const nextIdx = Math.min(idx + 1, cardEls.length - 1); // 마지막이면 그대로
      // 이모지가 살짝 뜬 뒤에 이동 느낌 나게 0.3초 정도 딜레이
      setTimeout(() => {
  ...
  fetch("/api/quick-feedback", {
    ...
    .then((res) => {
      if (res && res.ok) {
        if (isLike) {
          likeBtn.innerText = "👍 반영됨";
        } else {
          dislikeBtn.innerText = "👎 반영됨";
        }
